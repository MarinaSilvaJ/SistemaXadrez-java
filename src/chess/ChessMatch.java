...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		ChessPiece movedPiece = (ChessPiece)board.piece(target); //Referenciando peca na posicao destino para testar En Passant posteriormente
		
		promoted = null;
		if (movedPiece instanceof Pawn) {
			if ((movedPiece.getColor() == Color.WHITE && target.getRow() == 0) || (movedPiece.getColor() == Color.BLACK && target.getRow() == 7)) { //Teste para implementar movimento especial Promocao
		...
		//Testar  apos troca de turno se peca movida é um peao e moveu duas casas para implementar movimento En Passant
		if (movedPiece instanceof Pawn && (target.getRow() == source.getRow() - 2 || target.getRow() == source.getRow() + 2)) {
			enPassantVulnerable = movedPiece; //Se sim, marcar peca como vulneravel
		} 
		else {
	...
	//Metodo para substituir a peca promovida
	...
	//Metodo auxiliar para testar qual peca instanciar quando houver movimento especial Promocao
	...
	private Piece makeMove (Position source, Position target) {
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == null) { //Se posicao da coluna de destino for diferente da coluna de origem (movimento diagonal) e se nao houver peca capturada pois en passant nao posiciona no lugar da peca que foi capturada
	...
	private void undoMove(Position source, Position target, Piece capturedPiece) {
		...
		// Desfazer movimento da peca Torre no Roque Pequeno
		if (p instanceof King && target.getColumn() == source.getColumn() + 2) { //Se peca que esta sendo movimentada for Rei e e posicao destino for igual posicao origem + 2, movimentar a peca Torre para posicao do movimento Roque Pequeno
			Position sourceT = new Position(source.getRow(), source.getColumn() + 3);
			Position targetT = new Position(source.getRow(), source.getColumn() + 1);
		...
		// Desfazer movimento da peca Torre no Roque Grande
		if (p instanceof King && target.getColumn() == source.getColumn() - 2) { //Se peca que esta sendo movimentada for Rei e e posicao destino for igual posicao origem - 2, movimentar a peca Torre para posicao do movimento Roque Grande
			Position sourceT = new Position(source.getRow(), source.getColumn() - 4);
			Position targetT = new Position(source.getRow(), source.getColumn() - 1);
			ChessPiece rook = (ChessPiece)board.removePiece(targetT);
		...
		if (p instanceof Pawn) {
			if (source.getColumn() != target.getColumn() && capturedPiece == enPassantVulnerable) { //Se posicao da coluna de destino for diferente da coluna de origem (movimento diagonal) e se peca capturada for vulneravel a el passant
			 	ChessPiece pawn = (ChessPiece)board.removePiece(target);//Removendo a peca que foi devolvida na posicao incorreta
				Position pawnPosition ;
			 	if (p.getColor() == Color.WHITE) {
			 	...
			 	board.placePiece(pawn, pawnPosition); //Devolvendo peca para posicao correta
			}
		}
	}
